...
- **Добавить сервер**: пункт `+ Add server`, два поля (имя; ip или
  ...
  перезагрузки.
- **Править / удалить сервер**: выбери сервер стрелками и нажми `e`
  (править — пустое поле оставляет старое значение) или `d` (удалить, с
  подтверждением `y`). Пишется на флешку так же; на read-only носителе
  (CD в VM) изменить нельзя.
...
- **Конфиг `tc.conf`** (необязательный): рядом с `servers.conf` на флешке.
  ...
  - `RDP_EXTRA` — доп. флаги xfreerdp через пробел (напр. `/sound:sys:alsa`,
    `/bpp:32`);
  - `AUTOCONNECT=<ip|имя>` — киоск: при загрузке один раз сразу подключиться
    к серверу, после выхода из сессии — обычное меню.
