...
- **Settings** — единый админ-раздел (при желании под паролем, см. ниже).
  ...
  - **Система**: *Diagnostics* (интерфейсы/IP, маршруты, DNS, ping до шлюза,
    последние ошибки RDP-сессии — без ухода в шелл), *Console* (root-bash).
    Имя хоста задаётся ключом `HOSTNAME` в `tc.conf` (в меню его нет).
