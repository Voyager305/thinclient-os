...
|---|---|---|---|
| `thinclient.iso` | тест в виртуалке, быстрые демо | balenaEtcher / CD в VM | только вшитый (read-only) |
| `usb.img` | боевой на флешку | `dd` / `./flash-usb.sh` | редактируется из меню |
...
- **Добавить сервер**: пункт `+ Add server`, два поля (имя; ip или
  ip:порт). На usb.img сохраняется на флешку, на ISO — до перезагрузки.
- **Правка списка руками — только на usb.img**: воткни флешку в любой
  компьютер и отредактируй `servers.conf` на FAT-разделе (формат `имя;ip`,
  строки с `#` — комментарии). balenaEtcher пишет `.img` так же, как ISO,
  так что для «флешки с редактируемым списком» бери именно usb.img.
- **На флешке с ISO список не правится**: hybrid ISO кладёт на флешку
  файловую систему ISO9660 — она только для чтения по своей природе
  (формат компакт-диска), сохранить изменения в `servers.conf` не даст ни
  одна ОС. Список для ISO фиксируется при сборке: правь
  `board/thinclient/servers.conf.sample` и пересобирай.
...
board/thinclient/
  ...
  rootfs-overlay/
    ...
    etc/init.d/S50flash            монтирование носителя с servers.conf
